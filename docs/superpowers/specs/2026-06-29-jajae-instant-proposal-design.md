...
| 2D 평면도(이미지 #2) | **템플릿 라이브러리** — **10·20·30·40·50평대** 전 범위, 상담에서 나온 평수로 진행. 평형대 + 방/욕실 수로 표준 한국 아파트 평면도 매칭 → 라벨/치수 자동 주석 |
| 자재 디테일 패널 | **큐레이트 마감재 카탈로그**(신규) — 카테고리 × 티어 × **브랜드**. 예산이 티어를 결정. **국내 인테리어 자재 브랜드 DB를 시드로 저장**(대표님 제공 리스트), 제품·단가 등 미보유 데이터는 후속 업데이트 |
| 전달 방식 | **화면 프레젠테이션 + PNG/PDF + 공유 링크** (3종 모두). **공유 링크는 소비자 비밀번호 + 만료 적용** |
...
// 국내 실제 자재 카테고리에 맞춰 확장(이미지 #1 패널 + 추가 사양 + 종합 마감)
export type FinishCategory =
  ...
  | "door"              // 도어
  | "kitchen"           // 주방/싱크대
  | "sanitaryware"      // 욕실 위생도기
  | "lighting"          // 조명
  | "film"              // 인테리어필름
  | "board"             // PB/MDF 보드
  | "engineered_stone"  // 인조대리석(엔지니어드스톤)
  | "furniture"         // 붙박이장/가구
  | "molding";          // 몰딩/걸레받이

export interface MaterialBrand {
  id: string;
  name: string;              // "LX하우시스" · "구정마루" · "한샘" …
  categories: FinishCategory[]; // 다중(대기업 종합) 가능 — DB는 조인 테이블
  isImport: boolean;         // 수입 여부(벤자민무어·TOTO·이케아 등)
  segment: "major" | "specialist" | "distributor"; // 대기업 종합 / 전문 / 수입유통
  note?: string;
}
...
export interface FinishMaterial {
  ...
  tier: FinishTier;
  brandId: string;          // → MaterialBrand
  label: string;            // "강화마루 오크색"
  unitPrice: number;        // 대표가(원). 미보유 시 0 + priceStatus="estimated" → 후속 업데이트
  priceStatus: "confirmed" | "estimated";
  swatchUrl?: string;       // /swatches/flooring-oak.jpg (없으면 카테고리 기본 스왓치)
...
export interface ApartmentTemplate {
  id: string;                     // "apt-20s-3room-2bath"
  pyeongBand: 10 | 20 | 30 | 40 | 50; // 평형대
...
export interface Proposal {
  ...
  totalKRW: number;
  status: "draft" | "shared";
  shareToken?: string;            // 공개 링크용(랜덤)
  shareExpiresAt?: string;        // 만료 시각
  // sharePasswordHash 는 DB 전용(서버) — 타입/응답에 노출 안 함
  createdAt: string;
}
```
...
**`0017_material_catalog.sql`** — 자재 브랜드 DB + 마감재 카탈로그 + 시드
- `material_brands(id, name text unique, is_import boolean, segment text check('major','specialist','distributor'), note, sort int, created_at)`
- `material_brand_categories(brand_id uuid → material_brands(id), category text check(<FinishCategory>), primary key(brand_id, category))` — 다대다(대기업 종합 브랜드는 여러 카테고리)
- `finish_materials(id, category text check(...), tier text check('economy','standard','premium'), brand_id uuid → material_brands(id), label, unit_price numeric default 0, price_status text check('confirmed','estimated') default 'estimated', swatch_url, spec, sort int, created_at)`
- **시드(1차, 확정 데이터)**: 아래 "자재 브랜드 시드" 전체를 `material_brands` + `material_brand_categories`에 적재. `finish_materials`는 카테고리×티어 대표 1행씩(브랜드 연결, `price_status='estimated'`, 단가/스왓치 후속) — 패널이 즉시 렌더되도록.
- **RLS**: 인증 사용자 read-only(`authenticated select`), 쓰기는 admin/service. anon revoke.

**`0018_proposals.sql`** — 제안 산출물 + 비밀번호/만료 공유
- `pgcrypto` 확장(비밀번호 해시 `crypt()`/`gen_salt('bf')`).
- `proposals(id, estimate_id uuid → interior_estimates(id), contractor_id uuid → profiles(id), customer_name, template_id text, finishes jsonb, snapshot_url text, total_krw bigint, status text check('draft','shared') default 'draft', share_token text unique, share_password_hash text, share_expires_at timestamptz, created_at)`
- **RLS**: contractor 본인/admin select·insert·update(`contractor_id = auth.uid()`), service_role 신뢰, anon revoke. (0016 패턴 동일)
- **공개 공유 RPC** `get_shared_proposal(p_token text, p_password text)` **SECURITY DEFINER**:
  `status='shared'` **AND** `share_expires_at > now()` **AND** `share_password_hash = crypt(p_password, share_password_hash)` 인 경우에만 안전 컬럼(JSON) 반환, 아니면 null. RLS를 넓히지 않고 토큰+비번+만료를 한 함수에서 검증. anon `execute` 허용은 이 함수에 한정.

> 스튜디오 영속(design_scenes)은 본 제안 플로우에 불필요(씬은 인메모리 생성→스냅샷). 충돌 없음.

### 자재 브랜드 시드 (대표님 제공 — 0017 적재 원본)

> 카테고리 다중 = 종합 대기업. `삼porcelain`은 표기 불명 → **삼영세라믹**으로 적재(후속 확인). LG지인=LX Z:IN(LX하우시스 브랜드)이나 제공 리스트대로 별도 행 유지.

| segment | 브랜드 | 카테고리 | 수입 |
|---|---|---|---|
| major | LX하우시스 | flooring·wallpaper·window·film·board·engineered_stone | |
| major | KCC | paint·window·board·engineered_stone | |
| major | 현대L&C | flooring·wallpaper·window·film·engineered_stone | |
| major | 한솔홈데코 | flooring·board·door | |
| major | 동화기업 | flooring·board·door | |
| specialist | 구정마루·이건마루·동화자연마루·풍산마루 | flooring | |
| specialist | 한솔포레보드 | flooring·board | |
| specialist | 개나리벽지·신한벽지·서울벽지·DID벽지·LG지인벽지 | wallpaper | |
| specialist | 삼화페인트·노루페인트·강남제비스코·KCC페인트 | paint | |
| specialist | 벤자민무어코리아 | paint | ✔ |
| specialist | 삼영세라믹·동서타일·대보세라믹·윈세라믹 | tile | |
| distributor | 타일러스·윤현상재 | tile | ✔ |
| major | 한샘 | kitchen·furniture·sanitaryware | |
| specialist | 에넥스·리바트(현대리바트) | kitchen·furniture | |
| specialist | 대림바스·계림요업 | sanitaryware | |
| specialist | 아메리칸스탠다드·로얄토토(TOTO) | sanitaryware | ✔ |
| specialist | 필룩스·비츠조명·라이마스·두코 | lighting | |
| distributor | 이케아 | lighting·furniture | ✔ |
...
| 파일 | 함수 | 폴백 | 테스트 |
|---|---|---|---|
| `templates/index.ts` | `matchTemplate(brief): ApartmentTemplate` — 평형대(10~50)+방/욕실 최근접 매칭 | 미스 시 최근접 평형대 + 라벨/치수 재주석 | **단위(핵심)** |
| `templates/data.ts` | `APARTMENT_TEMPLATES[]` — **10·20·30·40·50평대** 표준 평면(평형대별 대표 구성, 방 슬롯+가구). 1차 평형대당 1~2개, 점진 확장 | 데이터 | 단위(무결성) |
...
수량근사는 템플릿 면적 기반(예: flooring/wallpaper/paint=바닥·벽면적, door/furniture=침실 수, tile=욕실·발코니 수×벽면적, kitchen/sanitaryware=개소 수). 카테고리별 규칙은 plan에서 상세화.
...
- `POST /api/proposal/[id]/share` — `{password, expiresInDays}` → `status='shared'` + `share_token`(랜덤) + `share_password_hash`(pgcrypto) + `share_expires_at` 설정 → 공유 URL 반환
- `POST /api/proposal/shared/[token]` — `{password}` → 공개 RPC `get_shared_proposal(token,password)` 검증 후 제안 JSON 반환(만료/오답 시 한국어 에러)
- `GET  /p/[token]` (페이지) — 비밀번호 입력 폼 → 위 라우트로 검증 → 읽기 전용 제안 표시(만료 안내)
...
| Phase | 내용 | 검증 |
|---|---|---|
| A | 타입 가산 + `0017_material_catalog`(brands+brand_categories+finish_materials, **브랜드 시드**, RLS) + `lib/proposal/materials.ts`(티어 선택) | **단위(티어/강등·시드 무결성)**·db RLS·typecheck |
| B | `lib/proposal/templates/`(10~50평대 데이터+매칭) + `floorplan-svg.ts`(이미지 #2) | **단위(매칭·SVG)** |
...
| E | `/api/proposal`·`[id]`·share(비번+만료) + `0018_proposals`(pgcrypto+RPC) + `/p/[token]` 공개 페이지 + PNG/PDF | db RLS(RPC 검증)·E2E |
...
- **RLS** `material_brands`/`finish_materials`(read-only)·`proposals`(소유자 격리)·`get_shared_proposal` RPC(shared·만료 전·비번 일치만 노출, 만료/오답 null) → PGlite db 테스트.
- **시드 무결성**: 브랜드 시드 적재 검증 + 카테고리별 finish_materials 최소 1행(티어 누락 없음) → 단위.
...
| 공유 링크 노출 | SECURITY DEFINER RPC로 `shared`·**비밀번호 일치**·**만료 전**·안전 컬럼만 반환. 랜덤 토큰 + bcrypt 해시, 비번/토큰 DB 평문 미저장 |
...
4. 화면 프레젠테이션 + PNG/PDF 내보내기 + 공유 링크(**비밀번호+만료** 공개 읽기) 동작
5. `material_brands`(브랜드 시드 적재)·`proposals`/`finish_materials` RLS 증명, 키 미설정에도 빌드·테스트 통과
